...
typedef struct
{
	char resEmail[50], cusName[50], cusPhoneNo[20], paymentMethod[20], cardNo[20], nameOnCard[20], expireDate[50], cvv[5];
	float totalPrice;
} resOrderDetails;
typedef struct
{
...
int profile(char filePath[])
{
		...
		if (choice == 1)
		{
			...
			if (strcmp(paymentMethodChoice, "yes") == 0)
			{
				...
				if (strcmp(hotelBooking.paymentMethod, "card") == 0)
				{
					...
					nameValid(hotelBooking.nameOnCard) ; 
			...
			else
			{
				...
				if (strcmp(paymentMethodChoice, "yes") == 0)
				{
					...
					if (strcmp(resOrder.paymentMethod, "card") == 0)
					{
						...
						printf("\nName on Card: ");
						fflush(stdin);
						gets(resOrder.nameOnCard);
						nameValid(resOrder.nameOnCard) ; 
						printf("expire Date (Format should be like Day/Month/Year)");
						fflush(stdin);
						gets(resOrder.expireDate);
						printf("Enter card cvv: ");
						fflush(stdin);
						gets(resOrder.cvv);
						fprintf(filePtr, "\nCard Details: ");
						...
						fprintf(filePtr, "\nName on Card: %s", resOrder.nameOnCard);
						fprintf(filePtr, "\nExpire Date: %s", resOrder.expireDate);
						fprintf(filePtr, "\nCvv: %s", resOrder.cvv);
						fprintf(resFilePtr, "\nCard Details: ");
						fprintf(resFilePtr, "\nCard No : ");
						for (i = 0; i < 16; i++)
	...
	else if (counter > 3 && role == 'a')
	{
		...
		fclose(filePtr);
		printf("Which hotel/restaurant account you want to remove: ");
		char toBeRemove[50];
		fflush(stdin);
		gets(toBeRemove);
		...
		if (accountMatch == 1)
		{
			...
			printf("resName: %s , length: %d,  linesOfData: %d", resName, strlen(resName), linesOfData);
			fclose(filePtr);
			i = 0;
			...
			fclose(filePtr);
			printf("resNamePos: %d", resNamePosInHome);
			i = 0;
			filePtr = fopen("home page.txt", "r");
			j = 0;
			while (fgets(line, sizeof line, filePtr) != NULL)
		...
		if (accountMatch == 0)
		{
			...
			if (accountMatch == 1)
			{
				...
				printf("resName: %s , length: %d,  linesOfData: %d", hotelName, strlen(hotelName), linesOfData);
				fclose(filePtr);
				i = 0;
				...
				fclose(filePtr);
				printf("resNamePos: %d", hotelNamePosInHome);
				i = 0;
				filePtr = fopen("home page hotels.txt", "r");
				j = 0;
				while (fgets(line, sizeof line, filePtr) != NULL)
